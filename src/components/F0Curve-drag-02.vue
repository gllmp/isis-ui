...
// Maps to store unit rectangles and boundary lines
...
const drawUnitRectangles = (svg: d3.Selection<SVGSVGElement, unknown, null, undefined>, xScale: d3.ScaleLinear<number, number>) => {
  if (!props.data) return;
      ...
      .on('drag', function (event, d) {
        ...
        props.data.units.forEach((unit, uIndex) => {
          ...
          if (unit.f0.segments.length > 0) {
            // Update the first segment's start_time
...
.interaction-group .boundary-line {
  /* Ensures boundary lines are on top */
  z-index: 10;
}
...
.interaction-group rect {
  /* Ensure that rects are below boundary lines */
  pointer-events: all;
}
</style>
